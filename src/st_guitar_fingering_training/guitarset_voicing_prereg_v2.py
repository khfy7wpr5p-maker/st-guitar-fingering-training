...
GUITARSET_OBSERVED_VOICING_MODEL_VERSION = "GUITARSET-OBSERVED-VOICING-MODEL.v2"
GUITARSET_VOICING_FEATURE_VERSION = "GUITARSET-VOICING-FEATURES.v2"
...
GUITARSET_VOICING_MAX_FRET = 20
GUITARSET_SOURCE_OBSERVED_MAX_FRET = 19
GUITARSET_NEGATIVE_SAMPLE_CAP = 32

FEATURE_SPECS = (
    ("open_ratio", "open_note_count/note_count"),
    ("mean_fret", "mean(fret)/20"),
    ("max_fret", "max(fret)/20"),
    ("min_positive_fret", "min(fret>0)/20 else 0"),
    ("max_positive_fret", "max(fret>0)/20 else 0"),
    ("positive_fret_span", "(max(fret>0)-min(fret>0))/20 else 0"),
    ...
    ("string_1_fret", "fret/20 if string 1 occupied else 0"),
    ("string_2_fret", "fret/20 if string 2 occupied else 0"),
    ("string_3_fret", "fret/20 if string 3 occupied else 0"),
    ("string_4_fret", "fret/20 if string 4 occupied else 0"),
    ("string_5_fret", "fret/20 if string 5 occupied else 0"),
    ("string_6_fret", "fret/20 if string 6 occupied else 0"),
...
def protocol_payload() -> dict:
    return {
        "schema": "st-guitar-guitarset-observed-voicing-model-prereg-v2",
        ...
        "candidate_set": {
            ...
            "candidate_enumeration": "ALL_EXACT_PHYSICAL_ASSIGNMENTS_WITHIN_0_20",
        ...
        "training": {
            ...
            "negative_sampling": "UP_TO_32_ALTERNATIVES_BY_ASCENDING_SHA256(GUITARSET-NEGSEL.v2|voicing_id|canonical_candidate)",
        ...
        "domain_extension": {
            ...
            "observed_fret20_positive_gold_expected": 0,
            "claim": "CANDIDATE_DOMAIN_COMPATIBILITY_EXTENSION_NO_POSITIVE_FRET20_GOLD",
        ...
        "next_gate": "OBSERVED_VOICING_MODEL_V2_DEVELOPMENT_IMPLEMENTATION_AND_FIT",
    }
...
def assert_frozen_protocol() -> None:
    if feature_schema_sha256() != EXPECTED_FEATURE_SCHEMA_SHA256:
        raise AssertionError("GuitarSet v2 feature schema drift")
    if protocol_sha256() != EXPECTED_PROTOCOL_SHA256:
        raise AssertionError("GuitarSet v2 preregistration drift")
    protocol = protocol_payload()
    if protocol["runtime_connection_authorized"] or protocol["production_authorized"]:
        raise AssertionError("GuitarSet v2 preregistration must not authorize runtime or production")
    if protocol["final_access_authorized"]:
        raise AssertionError("GuitarSet v2 preregistration must not open untouched final")
    if protocol["domain_extension"]["fret20_quality_authority"]:
        raise AssertionError("GuitarSet v2 must not claim fret-20 positive-gold quality authority")
