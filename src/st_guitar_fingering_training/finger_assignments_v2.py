...
def _partition_group(group: FrettingGroup) -> tuple[tuple[FrettingGroup, ...], ...]:
    """Enumerate every contiguous target partition of one passable H-B lower-bound group.

    H-B says all targets in one group *can* share one barre finger. S1-H-C.v2
    treats that as an option, not a mandatory identity constraint. A partition cut
    means the adjacent target blocks are fretted by distinct fingers.
    """
    ...
    # A bit set between consecutive target strings indicates a split.
