...
from st_guitar_fingering_training.normalized_path_optimizer import evaluate_group_normalized_sequence_path_decoder
...
def main():
    ...
    for number, validation_ids in enumerate(folds, 1):
        ...
        normalized_path = evaluate_group_normalized_sequence_path_decoder(model, val_sources)
        if greedy.events != normalized_path.events:
            raise AssertionError("greedy/normalized-path event mismatch")
        results.append({"fold": number, "events": normalized_path.events, "greedy_top1": greedy.top1_accuracy, "normalized_path_top1": normalized_path.top1_accuracy, "advantage": normalized_path.top1_accuracy-greedy.top1_accuracy, "normalized_path_exact_source_rate": normalized_path.exact_source_rate})
    weights = np.asarray([x["events"] for x in results], dtype=float)
    advantages = [x["advantage"] for x in results]
    report = {
        "stage": "6F.1-group-normalized-path-v1",
        "source_files": len(sources),
        "families": len({s.family_id for s in sources}),
        "folds": a.folds,
        "checkpoint_retained": False,
        "fold_results": results,
        "macro": {"normalized_path_top1": float(np.mean([x["normalized_path_top1"] for x in results])), "greedy_top1": float(np.mean([x["greedy_top1"] for x in results])), "normalized_path_vs_greedy": float(np.mean(advantages)), "normalized_path_exact_source_rate": float(np.mean([x["normalized_path_exact_source_rate"] for x in results]))},
        "event_weighted": {"normalized_path_top1": float(np.average([x["normalized_path_top1"] for x in results], weights=weights)), "greedy_top1": float(np.average([x["greedy_top1"] for x in results], weights=weights)), "normalized_path_vs_greedy": float(np.average(advantages, weights=weights))},
        ...
        "validation_policy": "bounded DP over physical voicing states with per-current-chord candidate normalization; no observed validation string/fret labels are used for decoding",
