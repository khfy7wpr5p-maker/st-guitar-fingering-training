...
def test_e3_feature_contract_is_fixed_and_target_blind():
    assert len(STAGE7G_E3_FEATURE_NAMES) == 40
    assert len(set(STAGE7G_E3_FEATURE_NAMES)) == 40
...
def test_e3_geometry_semantics_include_string_topology_and_barre_proxy():
    voicing = (
        (64, 1, 0),
        (60, 2, 1),
        (55, 4, 1),
...
def test_e3_curriculum_levels_are_target_blind_and_deterministic():
    assert stage7g_e3_curriculum_level(
        chord_size=2,
        candidate_count=10,
...
def test_e3_rule_derived_supervision_cannot_masquerade_as_teacher_gold():
    rule = Stage7GE3Supervision(
        curriculum_level="L1",
    ...
    validate_stage7g_e3_supervision(rule)

    with pytest.raises(ValueError, match="unknown Stage 7G-E3 rule-derived property target"):
        validate_stage7g_e3_supervision(
            Stage7GE3Supervision(
                curriculum_level="L2",
                provenance="RULE_DERIVED_PROPERTY",
    ...
    with pytest.raises(ValueError, match="limited to L1/L2"):
        validate_stage7g_e3_supervision(
            Stage7GE3Supervision(
                curriculum_level="L3",
                provenance="RULE_DERIVED_PROPERTY",
    ...
    with pytest.raises(ValueError, match="Teacher-GOLD must be blinded"):
        validate_stage7g_e3_supervision(
            Stage7GE3Supervision(
                curriculum_level="L3",
...
def test_e3_teacher_gold_accepts_equal_or_unsure_without_coercion():
    supervision = Stage7GE3Supervision(
        curriculum_level="L4",
        provenance="TEACHER_GOLD",
        target_name="pairwise_guitaristic_preference",
    ...
    validate_stage7g_e3_supervision(supervision)


@pytest.mark.parametrize("property_name", STAGE7G_E3_RULE_PROPERTY_TARGETS)
def test_e3_rule_property_targets_are_descriptive_not_preference_labels(property_name: str):
    ...
    assert stage7g_e3_rule_property_value(
        property_name,
        open_geometry,
        compact_geometry,
    ) == "COMPACT"
...
def test_e3_contract_rejects_nonfinite_and_bad_dimensions():
    with pytest.raises(ValueError, match="geometry delta keys"):
        stage7g_e3_curriculum_level(
            chord_size=2,
            candidate_count=4,
